...
const brunoChair01 = (): TProductVariantDraftBuilder =>
  ProductVariantDraft.presets
    ...
    .attributes([
      ...
      AttributeDraft.random().name('color').value({ 'en-GB': '#B29880' }),
      AttributeDraft.random().name('colorlabel').value({ 'en-GB': 'Beige' }),
      AttributeDraft.random().name('finishlabel').value({ 'en-GB': 'Birch' }),
      AttributeDraft.random().name('finish').value({ 'en-GB': '#FFC28D' }),
      AttributeDraft.random()
        .name('color-filter')
        .value({
          key: '#F5F5DC',
